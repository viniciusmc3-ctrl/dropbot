...
app.get('/auth/callback', async (req, res) => {
  ...
  try {
    ...
    res.send(`<html><body style="font-family:monospace;padding:40px;background:#0d1117;color:#e6edf3"><h2>✅ Token gerado!</h2><p>Copie e cole nas Configurações:</p><input style="width:100%;padding:12px;font-size:14px;background:#161b22;color:#58a6ff;border:1px solid #30363d;border-radius:8px" value="${accessToken}" onclick="this.select()"/><br><br><a href="/" style="color:#00c853">← Voltar ao DropBot</a></body></html>`);
  } catch (err) { res.status(500).send('Erro: ' + err.message); }
});

async function getProductImage(shop, token, productId, variantId) {
  try {
    const variantRes = await axios.get(`https://${shop}/admin/api/2024-01/variants/${variantId}.json`, { headers: { 'X-Shopify-Access-Token': token } });
    const variant = variantRes.data.variant;
    if (variant.image_id) {
      const imgRes = await axios.get(`https://${shop}/admin/api/2024-01/products/${productId}/images/${variant.image_id}.json`, { headers: { 'X-Shopify-Access-Token': token } });
      return imgRes.data.image?.src || null;
    }
    const productRes = await axios.get(`https://${shop}/admin/api/2024-01/products/${productId}.json`, { headers: { 'X-Shopify-Access-Token': token } });
...
app.get('/api/shopify/orders', async (req, res) => {
  ...
  try {
    ...
    const r = await axios.get(`https://${shop}/admin/api/2024-01/orders.json?${params}`, { headers: { 'X-Shopify-Access-Token': token } });
    ...
    const ordersWithImages = await Promise.all(orders.map(async (order) => {
      ...
      if (item?.product_id && item?.variant_id) {
        ...
        if (imageUrl) order.line_items[0].image = { src: imageUrl };
      }
      return order;
    }));
    res.json({ orders: ordersWithImages });
  } catch (err) { res.status(err.response?.status || 500).json({ error: err.response?.data?.errors || err.message }); }
});
...
app.get('/api/shopify/test', async (req, res) => {
  ...
  if (!shop || !token) return res.status(400).json({ error: 'shop e token são obrigatórios' });
  try {
    const r = await axios.get(`https://${shop}/admin/api/2024-01/shop.json`, { headers: { 'X-Shopify-Access-Token': token } });
    res.json({ ok: true, shop: r.data.shop?.name });
  } catch (err) { res.status(err.response?.status || 500).json({ ok: false, error: err.response?.data?.errors || err.message }); }
});
